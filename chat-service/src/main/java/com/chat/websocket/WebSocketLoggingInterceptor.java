            ...
            if (command != null) {
                switch (command) {
                    case CONNECT:
                        ...
                        break;
                    case DISCONNECT:
                        logger.info("WebSocket DISCONNECT: sessionId={}", sessionId);
                        break;
                    case SUBSCRIBE:
                        logger.info("WebSocket SUBSCRIBE: sessionId={}, destination={}", sessionId, destination);
                        break;
                    case UNSUBSCRIBE:
                        logger.info("WebSocket UNSUBSCRIBE: sessionId={}, destination={}", sessionId, destination);
                        break;
                    case SEND:
                        logger.debug("WebSocket SEND: sessionId={}, destination={}", sessionId, destination);
                        break;
                    default:
                        logger.debug("WebSocket {}: sessionId={}", command, sessionId);
                        break;
                }
            }
        }
